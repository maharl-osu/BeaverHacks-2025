...
export default function({onRemove}) {
    ...
    function renderRegisteredClasses() {
        return registered.map((event, idx) => {
            ...
            return <RegisteredClassCard key={idx} onRemove={() => {unregister(event.classID, idx)}} zoom={event.zoomLink} creator={event["creatorName"]} time={start + " - " + end} date={month + " " + day + " (" + weekday + ")"} title={event.name} description={event.description} cost={event.cost} onViewDetails={()=> {setDetailEvent(idx)}} />
        })
    }
